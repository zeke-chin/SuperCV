...
def generate_identicon_bytes(seed) -> bytes:
    ...
    for y in range(5):
        for x in range(5):
            if pattern[y * 5 + x]:
                ...
                draw.rectangle([x * block_size, y * block_size, (x + 1)
                               * block_size, (y + 1) * block_size], fill=color)
    ...
    for y in range(5):
        for x in range(5):
            if pattern[y * 5 + x]:
                ...
                draw.rectangle(
                    [size - (x + 1) * block_size, y * block_size,
                     size - x * block_size, (y + 1) * block_size],
                    fill=color)
                draw.rectangle(
                    [x * block_size, size - (y + 1) * block_size,
                     (x + 1) * block_size, size - y * block_size],
                    fill=color)
